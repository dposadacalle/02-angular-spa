...
export class HeroesService {
     ...
     public getHeroe(idx : string) : string{
     ...
     public buscarHeroes(termino: string) : IHeroe[]{
      ...
      for(let i = 0; i < this.heroes.length; i++){
        ...
        // IndexOf: Regresa un (NUMBER) 
        // La posicion donde encuentra el termino
        if(nombre.indexOf(termino) >= 0){
          heroe.index = i; // Guarda el index del elemento encontrado en la busqueda
          heroesArr.push(heroe); // inserto en Array (temporal)
        }
      }
